...
namespace VMBrevis.Manipulador
{
    ...
    public abstract class Convercoes
    {
        internal virtual string ConverteDeTParaString<T>(T classe) where T : class
        {
            ...
            List<FieldInfo> fields = classe.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance).Where(w => !w.Name.Equals("tabela") && !w.Name.Contains("proc") && !w.Name.Contains("esquema") && w.CustomAttributes.Count() == 0).ToList();
            fields.AddRange(classe.GetType().BaseType.GetFields(BindingFlags.NonPublic | BindingFlags.Instance).Where(w => !w.Name.Contains("Id")));
        ...
        internal virtual string ConverteTypeParaString(object objetoDeConvercao)
        {
            ...
            else if (objetoDeConvercao.GetType() == typeof(DateTime))
            {
                ...
                return "'" + dataHora.Year + "-" + dataHora.Month + "-" + dataHora.Day + " " + dataHora.Hour + ":" + dataHora.Minute + ":" + dataHora.Second + "'";
            }
                ...
                case Operacao.Excluir:
                    {
                        ...
                        parametros.Add(new SqlParameter { ParameterName = "Id_valor", Value = classe.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance).Where(w => w.Name.Contains("id_")).Select(w => w).FirstOrDefault().GetValue(classe) });
                        break;
                    }
        ...
        internal virtual string RetornaNomesDeMetodosDeTSeparadoPorVirgula<T>(T classe) where T : class
        {
            ...
            List<FieldInfo> fields = classe.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance).Where(w => !w.Name.Contains("esquema") && !w.Name.Equals("tabela") && !w.Name.Contains("proc") && w.CustomAttributes.Count() == 0).ToList();
            fields.AddRange(classe.GetType().BaseType.GetFields(BindingFlags.NonPublic | BindingFlags.Instance).Where(w => !w.Name.Contains("Id")));
                ...
                while (dadosDoBanco.Read())
                {
                    ...
                    foreach (FieldInfo field in fields)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (!object.Equals(dadosDoBanco[0], DBNull.Value))
                                {
                                    field.SetValue(dados, Convert.ToInt64(dadosDoBanco[0]));
                                }
                            }
                        }
                    }
